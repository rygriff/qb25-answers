...
ggplot(data = dt, aes(x = Liver)) +
  geom_histogram(breaks = seq(0, 70, 2)) +
   labs(title = "Median Expression Levels of Genes in The Liver",
       x = "Expression Level",
       y = "Frequency") +
  theme_minimal()

#the distribution is skewed towards 0, indicating that most genes have a median expression that is very low and only a few that are  - this is not specific to the liver as most organs/tissues would have key genes that need to be expressed and many many more that almost never get expressed.
```
...
ggplot(data = dt, aes(x = Liver + 1, y = `Heart - Left Ventricle` +1)) +
  ...
  labs(title = "Expression Levels in the Liver and Heart",
       x = "Expression Levels in Liver",
       y = "Expression Levels in Liver") +
  scale_x_log10() +
  scale_y_log10() +
  theme_minimal()
```
...
```{r q4}
ggplot(data = dt, aes(x = Liver + 1, y = `Heart - Left Ventricle` +1, colour = str_starts(Description,"MT"))) +
  geom_point(size = 2, alpha = 0.2) +
  geom_abline(slope = 1, color = "red", linetype = "dashed") +
  labs(title = "Expression Levels in the Liver and Heart",
       x = "Expression Levels in Liver",
...
ggplot(aes(x = value)) +
  geom_histogram(breaks = seq(0, 70, 2)) +
   labs(title = "Median Expression Levels of Genes in The Liver",
       x = "Expression Level",
       y = "Frequency") +
  facet_grid(.~ tissue_interest) +
  theme_minimal()
```

This requires first converting the data into "long" format with `pivot_longer()`:

# `{r} # dt_long <- dt %>% #   select("Description", "is_mitochondrial", all_of(focal_tissues)) %>% #   pivot_longer(cols = focal_tissues, names_to = "Tissue", values_to = "TPM") ##SEE ABOVE###`
...
dt %>% pivot_longer(
 ...
 mutate(is_mito = str_starts(Description, "MT-"))  %>% group_by(is_mito) %>% 
ggplot(aes(x = value, fill = is_mito)) +
  geom_histogram(breaks = seq(0, 70, 2)) +
   labs(title = "Median Expression Levels of Genes in The Liver",
       x = "Expression Level",
       y = "Frequency") +
  facet_grid(is_mito ~ tissue_interest, scales = "free_y") +
  theme_minimal()
```
